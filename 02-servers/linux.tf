...
resource "azurerm_network_interface" "linux_vm_nic" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.linux_vm_ip.id # Attach a public IP to this NIC
  }
}

# --- Define a public IP for the Linux VM ---
resource "azurerm_public_ip" "linux_vm_ip" {
  ...
  sku                 = "Basic"                                 # Basic SKU (cheapest option)
